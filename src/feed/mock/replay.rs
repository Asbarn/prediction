...
/// Replays a JSONL recording file, producing `RawMessage` items at configurable speed.
///
/// Each line of the JSONL file is expected to be a serialized `RecordLine` containing
/// the raw WebSocket frame text and a `local_ts` timestamp for pacing.
...
impl crate::feed::traits::RawDataSource for ReplayDataSource {
    async fn start(&self) -> anyhow::Result<mpsc::Receiver<RawMessage>> {
        ...
        tracing::info!(
            path = %path.display(),
            speed = speed,
            "starting JSONL replay"
        );

        // Read the entire file and parse all lines upfront
        let contents = tokio::fs::read_to_string(&path)
            .await
            .map_err(|e| anyhow::anyhow!("failed to read replay file {}: {}", path.display(), e))?;

        let mut lines: Vec<RecordLine> = Vec::new();
        ...
        tracing::info!(lines = total_lines, "parsed replay file");

        if lines.is_empty() {
            anyhow::bail!("replay file {} contains no valid JSONL lines", path.display());
        }
